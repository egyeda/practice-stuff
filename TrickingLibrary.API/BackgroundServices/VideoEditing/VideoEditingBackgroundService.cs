...
{
    public partial class VideoEditingBackgroundService : BackgroundService
    {
        ...
        public VideoEditingBackgroundService(
            ...
            ILogger<VideoEditingBackgroundService> _logger,
        ...
        protected override async Task ExecuteAsync(CancellationToken stoppingToken)
        {
            while (await _channelReader.WaitToReadAsync(stoppingToken))
            {
                ...
                catch (Exception ex)
                {
                    ...
                    _logger.LogError(ex, "Video Processing failed for: {0}", message.Input);
                }
                finally
                {
                    _videoManager.DeleteTemporaryVideo(message.Input);
